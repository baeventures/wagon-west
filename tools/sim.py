...
TOTAL=2600; FOOD_CAP=800; GOLD=18  # TOTAL overridden by KN at use sites
...
class Run:
    def __init__(s, prof, picks, seed):
        s.prof=prof
    ...
    def nearwater(s):
        return any(abs(m-s.miles)<130 for m,f in LM if f.get('river') or f.get('creek'))

def event(s, pace, dist, crossed):
    r=s.rng
    boost=0.12 if pace=='gruel' else 0.05 if pace=='stren' else 0
    if r.random()>0.35+boost: return
    pool=[]
    def fever():
    ...
    pool=[(3,fever),(2,breakdown),(2,goodgame),(2,spoil),(1,thieves),(2,fair),
          (0 if crossed else KN['stormw'],storm),(1,fire),(oxw,lame)]
    tot=sum(w for w,_ in pool); x=r.random()*tot
    for w,f in pool:
        x-=w
        if x<=0: f(); return

def cross_river(s, big):
    r=s.rng
    depth=round(r.uniform(5,9)) if big else round(r.uniform(2,7))
    fare=10+depth*4
...
def shop(s, mi):
    ...
    P={k:round(v*disc) for k,v in PRICE.items()}
    if KN['foxhalf'] and s.prof=='farmer': P['ox']=round(PRICE['ox']*0.5)
    # sell gold
    s.money+=round(s.gold*GOLD); s.gold=0
    # tonics to 2
    while s.tonics<2 and s.money>=PRICE['tonic']+10:
        s.money-=PRICE['tonic']; s.tonics+=1
    # spares to 2 each
    ...
    if mi>=1300:
        ...
        for g in prio:
            if g not in s.gear and s.money>=GEARP[g]+70: s.money-=GEARP[g]; s.gear.add(g)
...
def simulate(prof, picks, seed):
    s=Run(prof,picks,seed); r=s.rng
    ...
    PRICE2={k:round(v*OD) for k,v in PRICE.items()}
    if KN['foxhalf'] and prof=='farmer': PRICE2['ox']=round(PRICE['ox']*0.5)
    for k in ['wheel','axle','tongue']:
    ...
    while True:
        ...
        if s.miles>=KN['total']: s.cause='win'; return s
        se=s.sea(); cfg=SEA[se]
        ...
        req=(KN['total']-s.miles)/days_left
        ...
        new=min(KN['total'],s.miles+dist)
        crossed=None
        for m,f in LM:
            if s.miles<m<=new: crossed=(m,f); break
        s.miles=new
        event(s,pace,dist,crossed is not None)
        # plague
        ...
        if crossed:
            ...
            if f.get('store') and m not in visited:
                visited.add(m); shop(s,m)
            if f.get('river'):
                cross_river(s,f.get('big'))

def batch(prof,picks,n=1500,base=0):
    res=[simulate(prof,picks,base*100000+i) for i in range(n)]
...
import itertools
B={'total':2300,'huntA':7,'huntB':0.5,'wintox':0.012,'lame':0.75,'plague':0.008}
VARIANTS=[('PERKS: clerk10%, carp mend40%, farmer ox-half',dict(B,fsave=0,foxhalf=1,csave=0.4))]
configs=[
 ('clerk',['doc','hawk','marta'],'Clerk + Doc/Hawk/Marta'),
 ('carp',['doc','hawk','marta'],'Carpenter + Doc/Hawk/Marta'),
 ('carp',['hawk','marta','free'],'Carpenter + Hawk/Marta/free'),
 ('farmer',['hawk','marta','free'],'Farmer + Hawk/Marta/free'),
 ('farmer',['hawk','finch','free'],'Farmer + Hawk/Finch/free'),
 ('farmer',['free','free','free'],'Farmer + all free'),
]
key=[('clerk',['doc','hawk','marta'],'Clerk best-build'),('carp',['hawk','marta'],'Carpenter mid-build'),('farmer',['hawk','marta'],'Farmer lean-build')]
for vname,kn in VARIANTS:
    KN.update({'total':2600,'huntA':5,'huntB':0.42,'wintox':0.02,'stormw':2,'thin':0.6,'lame':1.0,'plague':0.008,'fsave':0.4,'foxhalf':0,'csave':0.3}); KN.update(kn)
    print(f"\n=== {vname} ===")
    print(f"{'CONFIG':26}{'WIN%':>6}{'MedDays':>9}{'Surv':>6}{'SawWinter%':>11}  TOP CAUSES")
    for prof,picks,label in key:
        o=batch(prof,picks,1200,hash(vname+label)%1000)
        causes=', '.join(f"{k}:{v}" for k,v in sorted(o['causes'].items(),key=lambda x:-x[1])[:3])
        print(f"{label:26}{o['win%']:>6.1f}{o['med_days']:>9}{o['avg_surv']:>6}{o['winter%']:>11}  {causes}")
